...
const CalclutCard = ({ title, isCheckout, submitOrder, customerInfo, setCustomerInfo }) => {
    ...
    return (
        <div className='border px-5 pt-5 pb-7.5 sticky top-20'>
            ...
            {isCheckout && <div>
                ...
                <ul className='flex flex-col gap-[18px]'>
                    {
                        products.map(({ id, price, title, quantity }) => {
                            const totalPrice = (price * quantity).toFixed(2)
                            return (
                                <li key={id} className='flex justify-between items-center'>
                                    <p className=' flex items-center gap-3'><span>{title}</span><span className='uppercase'>x {quantity}</span></p>
            ...
            {isCheckout && <div>
                ...
                <RadioGroup onValueChange={setPaymentMehod} defaultValue="check-payments" className="gap-2.5">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <Image src={'/images/payment-card.png'} width={124} height={43} alt='paypal' />
                    </div>
                </RadioGroup>
            </div>
            }
